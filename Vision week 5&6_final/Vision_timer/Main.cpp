	...
	*/
	...
	if(grayOn) {
		...
		*/
		...
		Image MatrixImage(originalImage);
		Translations trans;

		double** kernel = trans.createBackwardsKernel(false);

		//trans.MoveImage(grayImage, movedImage, kernel);
		//movedImage.SaveImageToFile("moved");
		trans.MatrixImage(originalImage, MatrixImage, 1, kernel, 0);
		MatrixImage.SaveImageToFile("matrix_");
	}
